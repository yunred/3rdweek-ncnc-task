...
const QuestionContainer = ({ faqType }: T.CustomerCenterProps) => {
  ...
  const [faqId, setFaqId] = useState<number | undefined>(0);
  const [QuestionsData,setQuestionsData] = useState<T.QuestionContainerProps[]>([]);

  useEffect(() => {
    const newId =
      tab === CustomerCenterContent.left
    ...
    useEffect(()=>{
      (async()=> {
          const res = await H.useFetch(C.FAQCONTENT_API + faqId); 
  ...
  return (
    <div className={style.Container}>
      ...
      <div className={style.body}>
        <div className={style.container}>
          {QuestionsData && QuestionsData.map((item :T.QuestionContainerProps) => {
